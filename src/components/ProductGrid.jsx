...
const ProductGrid = ({ title, products, categories }) => {
    ...
    <section className="container mx-auto px-6 py-10">
      ...
      <div className="grid grid-cols-1 sm:grid-cols-2 md:grid-cols-3 lg:grid-cols-4 xl:grid-cols-5 gap-x-6 gap-y-10">
        {productsToDisplay.map((product) => {
          // Ürünün kategorisini props olarak gelen categories listesinden bul
          const category = categories && categories.find(cat => cat.id === product.category_id);
          return (
            <ProductCard 
              key={product.id} 
              product={product} 
              category={category} // <<<< category prop'u ProductCard'a geçiriliyor
            />
          );
        })}
      </div>
