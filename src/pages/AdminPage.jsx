...
const AdminPage = () => {
    ...
    // GÜNCELLEME: `categories` artık form için formatlanmış olanı içeriyor.
    ...
    useEffect(() => {
        // GÜNCELLEME: Sayfa yüklendiğinde hem ürünler hem de form için kategoriler çekiliyor.
    ...
    const onSubmit = async (data) => {
        try {
            ...
            const formattedVariants = data.variants
                ...
                .filter(v => v.color.trim() && v.size.trim() && v.stock >= 0); // Stok 0 olabilir

            if (formattedVariants.length === 0) {
                toast.error("Lütfen en az bir geçerli ürün varyantı (Renk, Beden) ekleyin.");
                return;
            }
                            ...
                            <div className="space-y-3">
                                {variantFields.map((item, index) => (
                                    <div key={item.id} className="grid grid-cols-12 gap-2 items-center">
                                        ...
                                        <input {...register(`variants.${index}.size`)} placeholder="Beden" className="col-span-3 border p-2 rounded-md bg-gray-100" readOnly />
                                        <input type="number" {...register(`variants.${index}.stock`, { valueAsNumber: true, min: 0 })} placeholder="Stok" className="col-span-3 border p-2 rounded-md" />
